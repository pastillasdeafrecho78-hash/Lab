...
const mensajeSchema = z.object({
  contenido: z.string().min(1, 'Contenido es requerido').optional().nullable(),
  canalId: z.string().min(1, 'Canal es requerido'),
  archivoUrl: z.string().url().optional().nullable(),
  tipoArchivo: z.string().optional().nullable(),
  nombreArchivo: z.string().optional().nullable(),
  // Campos legacy para compatibilidad
  tipo: z.enum(['SUCURSAL', 'GENERAL', 'PRIVADO']).optional().nullable(),
  sucursalId: z.string().optional().nullable(),
  destinatarioId: z.string().optional().nullable()
}).refine(
  (data) => (data.contenido && data.contenido.trim().length > 0) || (data.archivoUrl && data.archivoUrl.trim().length > 0),
  {
  ...
  try {
    ...
    if (validatedData.canalId) {
      ...
      const mensaje = await prisma.mensaje.create({
        data: {
          contenido: validatedData.contenido?.trim() || (validatedData.archivoUrl ? `Archivo: ${validatedData.nombreArchivo || 'archivo'}` : ''),
          canalId: validatedData.canalId,
          tipo: 'GENERAL', // Default para compatibilidad
          archivoUrl: validatedData.archivoUrl || undefined,
          tipoArchivo: validatedData.tipoArchivo || undefined,
          nombreArchivo: validatedData.nombreArchivo || undefined,
          remitenteId: user.id
        },
        include: {
          remitente: {
